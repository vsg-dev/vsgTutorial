...
While the equivalent with the VulkanSceneGraph requires MyClass to be subclass from vsg::Object to add the itrusive reference counting:
...
In case 4 it is possible to fix the seg fault by moving the delete to after the last time that other_ptr is used, however if the processing section returns early or throws an exception the memory will be lost. Case 5 will work correctly as long as the processing section doesn't return early or throws an exception in which case it will leak the allocation object. Again case 6 is both cleaner and handles the early return case correctly, cleaning up any objects that have been allocated and no longer have an external reference.
...
vsg::ref_ptr<MyClass> ptr;
{
    ...
    // assign okect to the ref_ptr<> that increments its ref count to 1.
    ptr = &object;
